  ...
  application: () => {
    ...
    let appContent = {
      header: {
        ...
        active: true,
        version: "1.2",
        icon: "/opt/gui/images/icon/icon-terminal.png",
      ...
      main: (sender, nos) => {
        ...
        sender.ws.remoteFunction.desktop.terminalReady = (params) => {
          try {
            const { Shell } = bfs.require(nos.sysConfig.guiShell.manager);
            webShell = new Shell(
              //`%hostname:%pwd %username%roottag `,
              nos.sysConfig.guiShell.prompt,
              "Terminal",
              nos,
              (output) => {
              ...
              nos.sysConfig.guiShell.needLogin == 1 ? true : false // Authentication mode
              // sender.shell.remoteUserNeedLogin == 1 ? true : false // Authentication mode
            );
