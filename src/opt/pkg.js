    ...
    stack.negotiateKeyExchangeAsClient(
      ...
      (sharedKey, src) => {
        ...
        if (cmd === "status") {
            ...
            try {
              if (
                ...
                if (
                  ...
                  const loadedSysConfig = bfs.require(sysConfigPath, { noCache: true });

                  // let trustedSource =
                  //   (loadedSysConfig.packageManager.trustedSigners == signerFP);

                  // console.log(`signerFP: ${signerFP}\ntrustedSigners: ${loadedSysConfig.packageManager.trustedSigners}`);
                  let trustedSource = loadedSysConfig.packageManager.trustedSigners.includes(
                    signerFP.trim(),
                  );
                  if (!trustedSource) {
                    this.crt.textOut(
          ...
          // --- PATCH: --fp-update untuk update trustedSigners dari fingerprint repo (pkg <host> --fp-update) ---
          if (cmd == "fp-update") {
            if (!host) {
              this.crt.textOut("❌ Gunakan: pkg <host> --fp-update");
              this.shell.terminate();
              return;
            }
            // Kirim request list ke host, ambil fingerprint dari response
            stack.send(dst, JSON.stringify({ type: "list" }));
            stack.onDecryptedMessage(async (payload, src) => {
              try {
                if (typeof payload === "string" && payload.trim().startsWith("{")) {
                  const data = JSON.parse(payload);
                  if (data.type === "list-reply" && data.fingerPrintRepository) {
                    const fp = data.fingerPrintRepository;
                    const sysconfigPath = "/opt/conf/sysconfig.js";
                    let sysconfigRaw = this.fa.readFileSync(sysconfigPath, "utf8");
                    sysconfigRaw = sysconfigRaw.replace(
                      /(trustedSigners:\s*\[)[^\]]*(\])/,
                      `$1\n      \"${fp}\"\n    $2`
                    );
                    this.fa.writeFileSync(sysconfigPath, sysconfigRaw);
                    this.crt.textOut(`✅ trustedSigners di sysconfig.js berhasil diupdate ke:\n${fp}`);
                  } else {
                    this.crt.textOut("❌ Tidak dapat menemukan fingerprint di response server.");
                  }
                } else {
                  this.crt.textOut("❌ Response server tidak valid.");
                }
              } catch (e) {
                this.crt.textOut(`❌ Error parsing response: ${e}`);
              }
              this.shell.terminate();
            });
            return;
