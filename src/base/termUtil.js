...
class TermUtil {
  ...
  doTab() {
    ...
    if (isWin) baseDir = baseDir.replace(/\\/g, "/");
    let list = [];
    try {
      if (isWin && (baseDir === "/" || baseDir === "\\")) {
        // Tampilkan isi C:/ jika user cd / di Windows
        baseDir = process.cwd().split(path.sep)[0] + "/";
      }
      list = fs.readdirSync(baseDir);
    } catch (e) { }

    // Cari match dari autoCompletionList juga
    let autoCompleteMatches = [];
    if (this.autoCompletionList && this.autoCompletionList.length > 0 && !parentDir) {
      autoCompleteMatches = this.autoCompletionList.filter(name => name.startsWith(partial));
    }

    // Filter prefix
    let matches = list.filter(name => name.startsWith(partial));

    // Gabungkan hasil match filesystem dan autoCompletionList, hapus duplikat
    if (autoCompleteMatches.length > 0) {
      matches = [...new Set([...matches, ...autoCompleteMatches])];
    }
    ...
    matches = matches.map(name => {
      // Bersihkan trailing slash ganda yang mungkin ada
      const cleanName = name.replace(/\/+$/, "");

      // Cek apakah direktori
      let isDir = false;
      try {
        let stat = fs.statSync(path.join(baseDir, cleanName));
        isDir = stat && stat.isDirectory && stat.isDirectory();
      } catch (e) {
        // Jika ada error, periksa dari nama apakah mungkin direktori (ditandai dengan akhiran /)
        isDir = name.endsWith("/");
      }

      // Tambahkan satu slash jika direktori
      return isDir ? cleanName + "/" : cleanName;
    });

    // Pastikan tidak ada duplikasi (gunakan Set untuk mendeduplikasi)
