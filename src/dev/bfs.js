...
class NOSFileSystemDriver {
  ...
  loadNativeMountsFromConfig(
    configPath = isWin
      ? nosPath("opt", "conf", "mountpoints.json")
      : "/opt/conf/mountpoints.json"
  ) {
    let mounts = [
      // {
      //   virtual: "/serial",
    ...
    try {
      if (this.existsSync && this.readFileSync && this.existsSync(configPath)) {
        const raw = this.readFileSync(configPath, { "encoding": "utf8" });
  ...
  require(bfsPath, options = {}) {
    ...
    // PATCH: normalize path for Windows
    if (isWin) bfsPath = bfsPath.replace(/\\/g, "/");
    // Periksa apakah modul sudah ada di cache
    if (this.cache[bfsPath] && !options.noCache) {
      return this.cache[bfsPath];
    }
    const code = this.readFileSync(bfsPath, { "encoding": "utf8" });
    let result;
    try {
  ...
  _resolveNativeMount(filePath) {
    // PATCH: normalize path for Windows
    let normPath = filePath.replace(/\\/g, "/").replace(/\/+/g, "/");
  ...
  readFile(filePath, callback) {
    ...
    if (resolved) {
      return resolved.mount.handler.readFile(
        resolved.realPath,
        "utf8",
        callback
      );
  ...
  readFileSync(filePath, options = { "encoding": "utf8" }) {
    const resolved = this._resolveNativeMount(filePath);
    if (resolved) {
      return resolved.mount.handler.readFileSync(resolved.realPath, options);
    }
    const node = this.#getNodeByPath(filePath); // Menggunakan method private
    if (!node) {
      throw new Error(`File not found: ${filePath}`);
    ...
    const data = node.content
      ? Buffer.from(node.content, options.encoding)
      : "";
    // const data = node.content.toString(options.encoding || "utf8");
    // Cek checksum
    const checksum = crypto
      .createHash("sha256")
      .update(data)
  ...
  //   }
  //   const data = node.content
  //     ? Buffer.from(node.content).toString(isASCII === true ? encoding : encoding)
  //     : "";
  //   // const data = node.content.length > 0 ? Buffer.from(node.content) : Buffer.alloc(0);
  //   // Cek checksum
  //   const checksum = crypto
  //     .createHash("sha256")
  //     .update(Buffer.from(node.content))
  //     .digest("hex");
  //   if (node.checksum && node.checksum !== checksum) {
  //     throw new Error(`Checksum mismatch for file: ${filePath}`);
  //   }
  ...
  readBinaryFileSync(filePath) {
    const resolved = this._resolveNativeMount(filePath);
    if (resolved) {
      // Pastikan handler mengembalikan Buffer
      return resolved.mount.handler.readFileSync(resolved.realPath);
    }
    ...
    const data = node.content ? Buffer.from(node.content) : Buffer.alloc(0);
    // Cek checksum
    const checksum = crypto.createHash("sha256").update(Buffer.from(node.content)).digest("hex");
    if (node.checksum && node.checksum !== checksum) {
      throw new Error(`Checksum mismatch for file: ${filePath}`);
    }
    return data;
  ...
  writeFile(filePath, data, callback) {
    const resolved = this._resolveNativeMount(filePath);
    if (resolved) {
      return resolved.mount.handler.writeFile(
        resolved.realPath,
        data,
        "utf8",
        callback
  ...
  writeFileSync(filePath, data, encoding = "utf8") {
    const resolved = this._resolveNativeMount(filePath);
    if (resolved) {
      return resolved.mount.handler.writeFileSync(
        resolved.realPath,
        data,
        encoding
      );
  ...
  appendFileSync(filePath, data, encoding = "utf8") {
    const resolved = this._resolveNativeMount(filePath);
    if (resolved) {
      return resolved.mount.handler.appendFileSync(
        resolved.realPath,
        data,
        encoding
      );
  ...
  readdirSync(dirPath) {
    ...
    let normPath = dirPath.replace(/\\/g, "/").replace(/\/+/g, "/");
    if (normPath.length > 1 && normPath.endsWith("/"))
      normPath = normPath.replace(/\/+$/, "");
    ...
    const normPathWithSlash = normPath.endsWith("/")
      ? normPath
      : normPath + "/";
    for (const mnt of this.nativeMounts) {
  ...
  chownSync(filePath, user, group) {
    ...
    this.db
      .prepare(
        "UPDATE files SET owner_user = ?, owner_group = ? WHERE name = ?"
      )
      .run(user, group, path.basename(filePath));
  }
  ...
  chmodSync(filePath, mode) {
    ...
    this.db
      .prepare(
        "UPDATE files SET filemod_owner = ?, filemod_group = ?, filemod_other = ? WHERE id = ?"
      )
      .run(owner, group, other, node.id);
  }
