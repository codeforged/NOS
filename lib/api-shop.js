...
class NOSChannel extends chaSharekey {
  ...
  async accept(rsaKeyPair) {
    return new Promise((resolve) => {
      this.negotiateKeyExchangeAsServer(rsaKeyPair, (sharedKey, src) => {
        ...
        this._peer = src;
        if (this._eventHandlers["connect"]) this._eventHandlers["connect"](src, sharedKey);
  ...
  async connect(dst, makeSharedKeyFn) {
    return new Promise((resolve) => {
      this.negotiateKeyExchangeAsClient(dst, makeSharedKeyFn, (sharedKey, dstAddr) => {
        this._connected = true;
        this._sharedKey = sharedKey;
        this._peer = dstAddr;
        if (this._eventHandlers["connect"]) this._eventHandlers["connect"](dstAddr, sharedKey);
