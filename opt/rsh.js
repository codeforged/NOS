module.exports = {
	...
	main: function (nos) {
		...
		this.showSyntax = () => {
			this.display.textOut(`Syntax: ${args.command} <host>:<port> [-c <comm device>]`);
			this.shell.terminate();
		}
		const Flags = require(__dirname + "/../lib/packetFlags.js");
		class RemoteShell {
			constructor(address, port, parentShell, mqtnl, transmitData) {
				...
				this.conn = new mqtnl.mqtnlConnection(mqtnl.connMgr, Math.ceil(Math.random() * 65000 + 1), (data, sender) => {
					...
					try {
						if (data.header.packetHeaderFlag >= Flags.FLAG_SECURITY_NONE && data.header.packetHeaderFlag <= Flags.FLAG_SECURITY_NONE + 20) {
							// console.log("Incoming: "+ JSON.stringify(data));

							core_encryption.setSecurityAgent(mqtnl, Flags.flagToAgentName(data.header.packetHeaderFlag));
							// request to connect
							let rshData = { payload: "requestConnect", io: null }
							console.log("Connect from port: " + this.conn.port + " using " + Flags.flagToAgentName(data.header.packetHeaderFlag));
						...
						if (data.payload == "!connectAccept!") {
							...
							if (this.parentShell.keyboardActive === false) transmitData(data.payload);
						}
					} catch (e) {
						console.log(e)
					}
				...
				if (keys.includes("-force-aes")) {
					...
					this.conn.write(this.address, this.port, "", Flags.FLAG_SECURITY_HANDSHAKE_REQ);
				}
			}
		...
		if (args.params._ != null && args.params._.length > 0) {
			...
			if (typeof this.shell.addCompletion != "undefined") this.shell.addCompletion(this.dstAddress);
			this.remoteShell = new RemoteShell(this.dstAddress, this.dstPort, this.shell, this.mqtnl, (data) => {
				// clearTimeout(this.responseTimeout);        
				if (this.remoteShell.active == true)
