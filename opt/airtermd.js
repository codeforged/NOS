...
module.exports = {
  instanceName: "__rshserver",
  version: "1.3", // Increment version
  name: "Remote Shell Server (PacketStack)",
  needRoot: true,
  main: function (nos) {
    ...
    const { Shell } = require(`${nos.basePath}${activeShell}`);
    ...
    const createRemoteShellSession = (client) => {
      ...
      session.remoteShell.greeting(() => {
        try {
          const sacredPhrases = require(this.shell.basePath +
            "/lib/sacredPhrases");
