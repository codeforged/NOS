module.exports = {
  ...
  main: function (nos) {
    ...
    const chaSharekey = require(`${this.shell.basePath}/lib/api-shop.js`).chaSharekey;
    ...
    const conn = new mqtnl.mqtnlConnection(
      mqtnl.connMgr,
      srcPort,
      null,
      true
    ...
    packetStack.onDecryptedMessage((payload, src) => {
      ...
      if (payload === "!connectAccept!") {
        ...
        packetStack.send(src, JSON.stringify({ payload: `__termresize::${rows},${cols}` }));
    ...
    function saveKnownHost(fileAccess, knownHostsPath, host, port, fingerprint) {
      const hostport = `${host}:${port}`;
      let lines = [];
      if (fileAccess.fileExistsSync(knownHostsPath)) {
        lines = fileAccess.readFileSync(knownHostsPath).split("\n").filter(Boolean);
        // Remove old entry
    ...
    packetStack.negotiateKeyExchangeAsClient = async (dst, makeSharedKeyFn, onFinish) => {
      ...
      packetStack._sharedKeyHandler = async (type, src, payload) => {
        ...
        if (type === "pubkey") {
          ...
          if (knownHosts[hostport] && knownHosts[hostport] !== fingerprint) {
            ...
            let answer = await shell.userPrompt("Type 'yes' to accept and save this fingerprint: ", true);
            ...
            saveKnownHost(fileAccess, knownHostsPath, dst.address, dst.port, fingerprint);
            shell.crt.textOut("Fingerprint saved.\n");
          }
          const encrypted = crypto.publicEncrypt({
            key: pub,
            padding: crypto.constants.RSA_PKCS1_OAEP_PADDING,
          }, Buffer.from(sharedKey, "hex")).toString("hex");
    ...
    packetStack.negotiateKeyExchangeAsClient(dst, () => mySecretKey.toString("hex"), (sharedKey, src) => {
    ...
    this.shell.emitIOKey = (io) => {
      if (io.key.ctrl === true && io.key.name == 'c') { // Control + C
        ctrlCCount++;
