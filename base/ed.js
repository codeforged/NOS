  ...
  main: async function (nos) {
    ...
    const handleInput = (input) => {
      ...
      if (input.startsWith(".p ")) {
        ...
        if (!isNaN(start) && start >= 0 && start < lines.length && !isNaN(count) && count > 0) {
          for (let i = start; i < Math.min(start + count, lines.length); i++) {
            this.display.textOut(`${i + 1}: ${lines[i]}`);
          }
        } else {
          this.display.textOut("Format: .p N n  (N=baris awal, n=jumlah baris, atau $n untuk seluruhnya)");
        }
        prompt();
        return;
      }
      if (input === ".l") {
        this.display.textOut(`${lines.length} lines${changed ? ", changed" : ""}`);
        prompt();
        return;
      }
      if (input === ".clear") {
