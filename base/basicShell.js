const { TermUtil } = require(__dirname + "/termUtil");
const path = require("path");
const fs = require("fs");
const { shell } = require("../opt/conf/sysconfig");
...
class Shell {
  ...
  constructor(prompt = ">", title, nos, transmitData, authentication = false) {
    // const { TermUtil } = require(__dirname + "/base/termUtil");
    this.#nos = nos;
    this.version = "basicShell 0.06";
    this.prompt = prompt;
  ...
  set pwd(value) {
    this._pwd = value;
    let dirContents = fs.readdirSync(this.basePath + this._pwd);
  ...
  executeScript(lastCmd, isFromPipe = false) {
    this.lastCmd = lastCmd;
    let error = 0;
    try {
