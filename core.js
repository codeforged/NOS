...
class NOS {
  ...
  executeModule(modulePath, filename, callback, shell, root = true, params) {
    ...
    try {
      ...
      const xmodule = require(moduleFullPath);
      if (xmodule.needRoot === true && !root) {
        ...
        if (typeof xmodule.main === "function") {
          ...
          xmodule.main(root === true ? this : null); // Passing the NOS instance
  ...
  // shutdown(errorLevel) {
  //   for (let i = 0; i < this.runApps.length; i++) {
  //     if (this.runApps[i].instance.exitSignal) this.runApps[i].instance.exitSignal();
  //   }
  //   setTimeout(() => {
  //     process.exit(errorLevel);
  //   }, 100);
